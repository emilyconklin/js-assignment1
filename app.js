...
// Starts Text Content JS
const cardText = document.querySelector('#js-wrapper')

let html = ``
for (story of data) {
    html = html + `
    <div class="card" id="js-card1" style="background-color:${story.color}">
        <div class="heading-wrapper">
            <h3>${story.topic}</h3>
            <h4>${story.title}</h4>
        </div>
        <div class="button">Read for ${story.price}</div>
    </div>`
}
